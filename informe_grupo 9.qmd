...
pacman::p_load(
  ...
  reghelper, psych, sjPlot, ggplot2, ggeffects, lattice, kableExtra, sjmisc, equatiomatic, knitr, htmltools)
options(scipen = 999)
rm(list = ls())
```
...
reg2mujer_centrada <- lmer(indice_dem ~ 1 + imp_dem_cmc + grp_econ_cmc + mujer + cuan_dem_gmc + prom_imp_dem_gmc + (1 + mujer | país), data = datos, REML=F)

anova_tabla2 <- anova(results_3_centrados, reg2_centrada, reg2mujer_centrada, model_int)

rownames(anova_tabla2) <- c("Combinado (centrado)", "P.A. Imp_dem", "P.A. Mujer", "P.A. Imp_dem y Mujer")

kable(anova_tabla2, caption = "Tabla de devianza: comparación de modelos") %>%
  kable_styling(full_width = F)
