...
subtitle: "Análisis de datos multinivel 2025"
...
**Modelo teórico con hipótesis** ![](input/img/modelo_visual.png)
...
tab_des_1 <- datos_limpios %>% select(indice_dem, país, grp_econ, imp_dem, mujer) %>% 
  ungroup() %>%
...
```{r tbl-des_1, echo=FALSE, results='asis'}
#| label: tab_des_1
#| tbl-cap: "Tabla 1, descriptivos univariados variables nivel 1"
...
```{r tbl-des_2, echo=FALSE, results='asis'}
#| label: tab_des_2
#| tbl-cap: "Tabla 2, descriptivos univariados variables nivel 2"
#| tbl-cap-location: top
#| warning: false
 agg_datos <- datos %>% group_by(país) %>% summarise_all(mean) %>% as.data.frame()
agg_datos %>% select(cuan_dem, prom_imp_dem) %>%
...
$Yij = Y00 +Y10+Y20(mujerij)+ Y30(imp_demij)+uoj+ eij$
...
$Yij = Y00 +Y01(cuan_demj)+ Y02(prom_imp_demj)+uoj+ eij$
...
$Yij = Y00 +Y10+Y20(mujerij)+Y30(imp_demij)+Y01(cuan_demj) + Y02(prom_imp_demj)+uoj+ eij$
...
$Yij = Y00 +Y10+Y20(mujerij)+Y30(imp_demij)+Y01(cuan_demj) + Y02(prom_imp_demj)+uoj+u1j(imp_demij)+ eij$
...
$Yij = Y00 +Y10+Y20(mujerij)+Y30(imp_demij)+Y01(cuan_demj) + Y02(prom_imp_demj)+ Y21(mujerij ⋅ cuan_demj)+uoj+u1j(mujerij)+e$
...
Grafico1 <- sjPlot::plot_scatter(dat_scat, cuan_dem,indice_dem,
            dot.labels = to_label(dat_scat$país),
            fit.line = "lm",
            show.ci = TRUE
            ) +
   labs(x = "Índice de Adhesión a Principios de la Democracia Liberal",
       y = "Percepción nacional sobre cuán democrático es un país",
       title = "Relación entre cuan democratico es percibido un país con índice de adhesión")

```

```{r fig-1, echo=FALSE, results='asis'}
#| label: fig-1
#| tbl-cap: "Relación entre cuan democratico es percibido un país con índice de adhesión"
#| tbl-cap-location: top
#| warning: false
Grafico1
...
```{r  echo=FALSE, warning=FALSE, message=FALSE }
Grafico2 <- sjPlot::plot_scatter(dat_scat, prom_imp_dem,indice_dem,
            dot.labels = to_label(dat_scat$país),
            fit.line = "lm",
            show.ci = TRUE
            )+
   labs(x = "Índice de Adhesión a Principios de la Democracia Liberal",
       y = "promedio de importancia de vivir en democracia",
       title = "Relación entre importancia de la democracia por país con índice de Adhesión")

```

```{r fig-2, echo=FALSE, results='asis'}
#| label: fig-2
#| tbl-cap: "Relación entre importancia de la democracia por país con índice de adhesión"
#| tbl-cap-location: top
#| warning: false
Grafico2
```

La @fig-2 muestra una clara relación positiva entre el promedio nacional de importancia asignada a vivir en un país democrático y el índice de adhesión a principios de la democracia liberal. Esta pendiente positiva indicaría que en aquellos países donde la democracia es altamente valorada, también existe un mayor grado de internalización de sus principios fundamentales, como las elecciones libres, la igualdad ante la ley y la protección de derechos individuales.

A diferencia de la @fig-1, que mostraba la relación entre la percepción institucional de cuán democrático es un país y la adhesión a principios liberales, aquí se destaca el peso del clima político o cultura cívica.
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
tab_model <- tab_model(results_0, results_1_centrado, results_2_centrado, results_3_centrados, reg2_centrada, model_int, show.ci = FALSE, show.se = TRUE, dv.labels = c( "Nulo", "Var. Niv. 1 (centrado)", "Var. Niv. 2 (centrado)", "Combinado (centrado)", "Pendiente Aleatoria (centrado)", "Interacción entre niveles"))
```

```{r tbl-modelos, echo=FALSE, results='asis'}
#| label: tab_model
#| tbl-cap: "Tabla 3, Modelos centrados"
#| tbl-cap-location: top
#| warning: false
tab_model
```
...
#| label: fig-3
#| tbl-cap: "Interacción entre promedio de percepción de cuan democratico es un país con mujer"
#| tbl-cap-location: top
#| warning: false
Grafico3
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# grafico interacción
Grafico4 <-plot_model(model_int, type = "int") + labs(
    title = "Interacción",
    x = "Mujer",
    y = "Índice de percepción de valores de la democracia liberal"
  )
...
```{r fig-4, echo=FALSE, results='asis'}
#| label: fig-4
#| tbl-cap: "Interacción"
#| tbl-cap-location: top
#| warning: false
...
tabla4 <- kable(anova_tabla2, caption = "Tabla de devianza 1: Comparación Pendiente Fija con P.A. Imp_dem") %>%
  kable_styling(full_width = F)

```

```{r tbl-devianza1, echo=FALSE, results='asis'}
#| label: tabla4
#| tbl-cap: "Tabla 4, Devianza Modelo combinado y Modelo con pendiente aleatoria"
#| tbl-cap-location: top
#| warning: false
tabla4
...
tabla5 <- kable(anova_tabla3, caption = "Tabla de devianza 1: Comparación Pendiente Fija con P.A. Mujer") %>%
  kable_styling(full_width = F)

```

```{r tbl-devianza2, echo=FALSE, results='asis'}
#| label: tabla5
#| tbl-cap: "Tabla 5, Devianza Modelo combinado y Modelo de interacción"
#| tbl-cap-location: top
#| warning: false
tabla5
...
estex.m23 <- influence(reg2_centrada, "país") 
...
```{r fig-5, echo=FALSE, results='asis'}

#| label: fig-5
#| tbl-cap: "Distancia de cook"
#| tbl-cap-location: top
#| warning: false
...
**Modelo teórico con resultados** ![](input/img/modelo_teórico_resultados.png)
